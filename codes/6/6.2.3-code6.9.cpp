...
// File name: 6.cpp
// Description: wait and pop using conditional_variable
//
#include <iostream>
#include <memory>
...
int main()
{
    ...
    // really wait
    auto future_item = std::async(launch::async, [&tsq]() {
        return tsq.wait_and_pop();
    });
    auto push_thread = thread([&tsq](){
        tsq.push(4);
    });
    auto f_item = future_item.get();
    if (f_item)
        cout << "Popped: " << *f_item << endl;
    else
        cout << "Queue is empty!" << endl;
    push_thread.join();
